...
## Copying a reference site: take the form, never the unverifiable claims

Roman sent screenshots of a polished pool-builder site (2026-09-16) and asked
for the same look. The most eye-catching block on it is a row of stats:
**18+ YEARS BUILDING · 900+ POOLS COMPLETED · Lifetime SHELL WARRANTY**. Its
hero carries the same idea as checkmarks: *Licensed FL pool contractor ·
Lifetime structural warranty · Financing available*.

Every one of those is a factual claim about a business. None of them is true
of a microsite that has never done a job. Reproducing that block verbatim
with plausible-looking numbers is not "matching the design" -- it is the
fabricated-trust-claim line, arriving disguised as a layout request.

**The move is to keep the visual form and refill it with things that are
true.** A stat box is a big number in a heavy display face over a small
uppercase label; it does not care what the number counts. On the foundation
site the same band became:

| 4 | 18 | 5 | 1–3 |
|---|---|---|---|
| Counties covered | Areas written up | Distinct repairs | Days for most jobs |

Identical weight on the page, and every figure is checkable against the site
itself. The block reads as competence for the same reason the original does
-- specific numbers, confidently placed -- without asserting anything that
would be a lie if a customer relied on it.

Ask of any element borrowed from a reference: **is this a shape or a
claim?** Shapes (tab rails, photo panels, floating badges, eyebrow pills,
accent-coloured second lines, stat boxes) copy freely. Claims (years, jobs
completed, warranties, licensing, financing, review counts, team photos)
never copy -- they get replaced with something true or the slot goes to
something else.

Say which substitution was made and why, in the commit and to the owner.
Roman has never once pushed back when told "the reference claims X, we do
not have X, here is the true version" -- the objection people expect does
not actually materialise, and quietly copying the claim would have.
